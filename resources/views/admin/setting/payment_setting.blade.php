...
@section('content')
    ...
    <div class="row">
        <div class="col-md-10">
            @php
                // Only show these payment gateways: xendit, doku, paypal
                $allowed_gateways = ['xendit', 'doku', 'paypal'];
                $payment_gateways = App\Models\Payment_gateway::whereIn('identifier', $allowed_gateways)->get();
            @endphp
            <div class="ol-card">
                            ...
                            <div class="tab-pane fade @if(!isset($_GET['tab'])) show active @endif" id="v-pills-currency" role="tabpanel" aria-labelledby="v-pills-currency-tab" tabindex="0">
                                ...
                                <div class="ol-card-body">
                                    <form action="{{ route('admin.payment.settings.update') }}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="fpb-7 mb-3">
                                            <label class="form-label ol-form-label">{{ get_phrase('Currency decimal places') }}</label>
                                            <select class="form-control ol-form-control ol-select2" data-toggle="select2" name="currency_decimal_places" required>
                                                @php
                                                    $decimal_setting = get_settings('currency_decimal_places');
                                                    // Default to 2 if not set
                                                    if ($decimal_setting === null || $decimal_setting === '') {
                                                        $decimal_setting = '2';
                                                    }
                                                @endphp
                                                <option value="0" @if ($decimal_setting === '0') selected @endif>
                                                    {{ get_phrase('0 (Rp 10.000)') }}</option>
                                                <option value="2" @if ($decimal_setting === '2') selected @endif>
                                                    {{ get_phrase('2 (Rp 10.000,00)') }}</option>
                                            </select>
                                            <small class="form-text text-muted">
                                                {{ get_phrase('For Indonesian Rupiah, use 0 for informal display (Rp 10.000) or 2 for formal display (Rp 10.000,00)') }}
                                            </small>
                                        </div>
