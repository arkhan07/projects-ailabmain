...
<link rel="stylesheet" href="{{ asset('assets/frontend/default/landingpage/style.css') }}">
@endpush
...
<nav class="navbar" role="navigation" aria-label="Main navigation">
    <div class="navbar-container">
        ...
        <!-- Desktop Navigation -->
        <div class="nav-links">
            <a href="{{ route('courses') }}" class="nav-item @if ($current_route == 'courses') active @endif">
                <span class="nav-text">{{ get_phrase('Manifesto') }}</span>
                <span class="nav-dot" aria-hidden="true"></span>
            </a>
            <a href="{{ route('bootcamps') }}" class="nav-item @if ($current_route == 'bootcamps' || $current_route == 'bootcamp.details') active @endif">
                <span class="nav-text">{{ get_phrase('Ai Legend') }}</span>
                <span class="nav-dot" aria-hidden="true"></span>
            </a>
        </div>

        @guest
        <a href="{{ route('register.form') }}" class="btn-signup" role="button">
            <span class="btn-text">{{ get_phrase('Sign Up') }}</span>
            <span class="btn-icon">
                <img src="{{ asset('assets/frontend/default/landingpage/img/vector-74.svg') }}" alt="" class="arrow-icon" />
            </span>
        </a>
        @else
        <div class="user-menu-wrapper">
            <a href="{{ route('cart') }}" class="cart-link" data-bs-toggle="tooltip" data-bs-title="{{ get_phrase('Cart') }}">
                @php
                    $numberof_cart_item = App\Models\CartItem::where('user_id', auth()->user()->id)->count();
                @endphp
                @if ($numberof_cart_item > 0)
                    <span class="cart-badge">{{ $numberof_cart_item }}</span>
                @endif
            ...
            <div class="profile-dropdown">
                ...
                <div class="profile-dropdown-menu" id="profileDropdownMenu">
                    ...
                    @else
                        <a href="{{ route('my.courses') }}" class="dropdown-menu-item">
                            <svg width="16" height="16" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_975_218)">
                                    <path d="M3 0H1.66667C0.747667 0 0 0.747667 0 1.66667V2.33333C0 2.701 0.299 3 0.666667 3H3C3.36767 3 3.66667 2.701 3.66667 2.33333V0.666667C3.66667 0.299 3.36767 0 3 0ZM0.666667 2.33333V1.66667C0.666667 1.11533 1.11533 0.666667 1.66667 0.666667H3L3.00067 2.33333H0.666667ZM7.33333 5H5C4.63233 5 4.33333 5.299 4.33333 5.66667V7.33333C4.33333 7.701 4.63233 8 5 8H6.33333C7.25233 8 8 7.25233 8 6.33333V5.66667C8 5.299 7.701 5 7.33333 5ZM7.33333 6.33333C7.33333 6.88467 6.88467 7.33333 6.33333 7.33333H5V5.66667H7.33333V6.33333ZM6.33333 0H5C4.63233 0 4.33333 0.299 4.33333 0.666667V3.66667C4.33333 4.03433 4.63233 4.33333 5 4.33333H7.33333C7.701 4.33333 8 4.03433 8 3.66667V1.66667C8 0.747667 7.25233 0 6.33333 0ZM5 3.66667V0.666667H6.33333C6.88467 0.666667 7.33333 1.11533 7.33333 1.66667L7.334 3.66667H5ZM3 3.66667H0.666667C0.299 3.66667 0 3.96567 0 4.33333V6.33333C0 7.25233 0.747667 8 1.66667 8H3C3.36767 8 3.66667 7.701 3.66667 7.33333V4.33333C3.66667 3.96567 3.36767 3.66667 3 3.66667ZM1.66667 7.33333C1.11533 7.33333 0.666667 6.88467 0.666667 6.33333V4.33333H3L3.00067 7.33333H1.66667Z" fill="currentColor" />
                                </g>
                    ...
                    @endif
                    <a href="javascript:;" class="dropdown-menu-item logout-link">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <path d="M19 12H9M19 12L16 9M19 12L16 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
        ...
        <button class="hamburger" aria-label="Menu" aria-expanded="false">
            <img src="{{ asset('assets/frontend/default/landingpage/img/nav-mobile.svg') }}" alt="Menu" class="hamburger-icon" />
    ...
    <div class="mobile-menu">
        ...
        <div class="mobile-user-header">
            <img class="mobile-user-avatar" src="{{ get_image(Auth()->user()->photo) }}" alt="{{ Auth()->user()->name }}">
            <div class="mobile-user-info">
                <p class="mobile-user-name">{{ ucfirst(Auth()->user()->name) }}</p>
                <p class="mobile-user-role">{{ ucfirst(Auth()->user()->role) }}</p>
            </div>
        </div>
        @if (in_array(auth()->user()->role, ['admin', 'instructor']))
        <a href="{{ route(auth()->user()->role . '.dashboard') }}" class="mobile-nav-item">{{ get_phrase('Dashboard') }}</a>
        @else
        <a href="{{ route('my.courses') }}" class="mobile-nav-item">{{ get_phrase('Dashboard') }}</a>
        @endif
        <a href="javascript:;" class="mobile-nav-item logout-link">{{ get_phrase('Logout') }}</a>
        @else
        <a href="{{ route('courses') }}" class="mobile-nav-item">{{ get_phrase('Manifesto') }}</a>
        <a href="{{ route('bootcamps') }}" class="mobile-nav-item">{{ get_phrase('Ai Legend') }}</a>
        <a href="{{ route('login') }}" class="mobile-nav-item">{{ get_phrase('Login') }}</a>
        <a href="{{ route('register.form') }}" class="mobile-nav-item btn-mobile-signup">{{ get_phrase('Sign Up') }}</a>
        @endguest
    </div>
</nav>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (profileBtn && profileMenu) {
                ...
                profileMenu.addEventListener('click', (e) => {
                    if (!e.target.closest('.logout-link') && !e.target.closest('a')) {
                        e.stopPropagation();
                    }
                });
            }
