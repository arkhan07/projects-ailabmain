...
if (! function_exists('currency')) {
    function currency($price = false, $decimal_override = null)
    {
        $pattern = get_settings('currency_position');
        $symbol  = DB::table('currencies')->where('code', get_settings('system_currency'))->value('symbol');

        // Get decimal places setting (default to 2 if not set)
        $decimal_setting = get_settings('currency_decimal_places');

        // Handle null, empty string, and ensure integer value
        if ($decimal_override !== null) {
            $decimal_places = intval($decimal_override);
        } elseif ($decimal_setting !== null && $decimal_setting !== '') {
            $decimal_places = intval($decimal_setting);
        } else {
        ...
        // For Indonesian format: use . for thousands separator and , for decimal separator
        $currency_code = get_settings('system_currency');

        if ($currency_code == 'IDR') {
            // Indonesian Rupiah format: Rp 10.000 or Rp 10.000,00
            $formatted_price = number_format($price, $decimal_places, ',', '.');
        } else {
            // International format: $10,000 or $10,000.00
            $formatted_price = number_format($price, $decimal_places, '.', ',');
        }

        // Apply currency position
        if ($pattern == 'right') {
            $currency = $formatted_price . $symbol;
        } elseif ($pattern == 'left') {
            $currency = $symbol . $formatted_price;
        } elseif ($pattern == 'right-space') {
            $currency = $formatted_price . ' ' . $symbol;
        } elseif ($pattern == 'left-space') {
            $currency = $symbol . ' ' . $formatted_price;
        } else {
            // Default to left-space
            $currency = $symbol . ' ' . $formatted_price;
        }
