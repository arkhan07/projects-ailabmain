...
class LiveClassController extends Controller
{
    ...
    public function live_class_store(Request $request, $course_id)
    {
        ...
        if ($request->provider == 'zoom') {
            ...
            if (!is_array($meeting_info_arr)) {
                return redirect(route('admin.course.edit', ['id' => $course_id, 'tab' => 'live-class']))->with('error', get_phrase('Failed to create Zoom meeting. Please check your Zoom credentials.'));
            }

            if (array_key_exists('code', $meeting_info_arr)) {
                $error_message = $meeting_info_arr['message'] ?? 'Failed to create Zoom meeting';
                return redirect(route('admin.course.edit', ['id' => $course_id, 'tab' => 'live-class']))->with('error', get_phrase($error_message));
            }
    ...
    public function create_zoom_live_class($topic, $date_and_time)
    {
        ...
        if (empty($token)) {
            return json_encode([
                'code' => 401,
                'message' => 'Failed to authenticate with Zoom. Please check your Zoom credentials in settings.'
            ]);
        }
    ...
    public function create_zoom_token()
    {
        ...
        if ($curlError) {
            \Log::error('Zoom API CURL error: ' . $curlError);
            return null;
        }
        ...
        if (!is_array($oauthResponse) || !isset($oauthResponse['access_token'])) {
            \Log::error('Zoom API failed. HTTP Code: ' . $httpCode . ', Response: ' . $response);
            return null;
        }
