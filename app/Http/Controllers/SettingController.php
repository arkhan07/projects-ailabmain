...
class SettingController extends Controller
{
    ...
    public function website_settings_update(Request $request)
    {
        ...
        if ($request->type == 'frontend_settings') {
            // Remove 'type' field from data to prevent trying to update it
            unset($data['type']);
    ...
    public function update_home(Request $request, $id)
    {
        ...
        } elseif ($home_page == 'landingpage') {
            // Handle landingpage settings
            $speech = [
                // Hero Section
                ...
                'hero_image' => $request->input('previous_hero_image'),

                // Spotlight Logos
                'spotlight_logo_1' => $request->input('previous_spotlight_logo_1'),
                'spotlight_logo_1_alt' => $request->input('spotlight_logo_1_alt'),
                'spotlight_logo_2' => $request->input('previous_spotlight_logo_2'),
                'spotlight_logo_2_alt' => $request->input('spotlight_logo_2_alt'),
                'spotlight_logo_3' => $request->input('previous_spotlight_logo_3'),
                'spotlight_logo_3_alt' => $request->input('spotlight_logo_3_alt'),
                'spotlight_logo_4' => $request->input('previous_spotlight_logo_4'),
                'spotlight_logo_4_alt' => $request->input('spotlight_logo_4_alt'),

                // About AI Section
                'about_title' => $request->input('about_title'),
                'about_intro' => $request->input('about_intro'),
                'about_list_title' => $request->input('about_list_title'),
                'about_features' => $request->input('about_features', []),
                'about_closing' => $request->input('about_closing'),

                // What Section
                'what_title' => $request->input('what_title'),
                'what_intro' => $request->input('what_intro'),
                'what_list_title' => $request->input('what_list_title'),
                'what_features' => $request->input('what_features', []),
                'what_closing' => $request->input('what_closing'),

                // Why Different Section
                'why_title' => $request->input('why_title'),
                'why_list_title' => $request->input('why_list_title'),
                'why_features' => $request->input('why_features', []),
                'why_closing' => $request->input('why_closing'),

                // Access/Pricing Section
                'access_title' => $request->input('access_title'),
                'access_subtitle' => $request->input('access_subtitle'),
                'pricing' => $request->input('pricing'),
                'pricing_period' => $request->input('pricing_period'),
                'pricing_note' => $request->input('pricing_note'),
                'pricing_disclaimer' => $request->input('pricing_disclaimer'),

                // CTA Links
                'cta_button_text' => $request->input('cta_button_text'),
                'cta_button_link' => $request->input('cta_button_link'),

                // Section Visibility
                'show_spotlight' => $request->has('show_spotlight'),
                'show_about' => $request->has('show_about'),
                'show_what' => $request->has('show_what'),
                'show_why' => $request->has('show_why'),
                'show_pricing' => $request->has('show_pricing'),
            ];

            // Handle hero image upload
            if ($request->hasFile('hero_image')) {
                $image_name = uniqid() . '_hero.' . $request->file('hero_image')->getClientOriginalExtension();
                FileUploader::upload($request->hero_image, 'uploads/home_page_image/landingpage/' . $image_name);
                $speech['hero_image'] = 'uploads/home_page_image/landingpage/' . $image_name;

                // Remove old image
                $previous_image = $request->input('previous_hero_image');
                if (!empty($previous_image) && file_exists(public_path($previous_image))) {
                    remove_file(public_path($previous_image));
                }
            }

            // Handle spotlight logo uploads
            for ($i = 1; $i <= 4; $i++) {
                $fieldName = 'spotlight_logo_' . $i;
                if ($request->hasFile($fieldName)) {
                    $image_name = uniqid() . '_logo' . $i . '.' . $request->file($fieldName)->getClientOriginalExtension();
                    FileUploader::upload($request->file($fieldName), 'uploads/home_page_image/landingpage/' . $image_name);
                    $speech[$fieldName] = 'uploads/home_page_image/landingpage/' . $image_name;

                    // Remove old image
                    $previous_image = $request->input('previous_' . $fieldName);
                    if (!empty($previous_image) && file_exists(public_path($previous_image))) {
                        remove_file(public_path($previous_image));
                    }
                }
    ...
    // Amazon S3 Settings
    public function amazom_s3_settings()
    {
        $amazon_s3_setting = DB::table('settings')->where('type', 'amazon_s3')->first();

        if ($amazon_s3_setting && $amazon_s3_setting->description) {
            $page_data['amazon_s3_data'] = json_decode($amazon_s3_setting->description, true);
        } else {
            // Default empty values
            $page_data['amazon_s3_data'] = [
                'active' => null,
                'AWS_ACCESS_KEY_ID' => '',
    ...
    public function amazom_s3_settings_update(Request $request)
    {
        ...
        $amazon_s3_data = [
            'active' => 1, // Mark as active when saved
        ...
        DB::table('settings')
            ->updateOrInsert(
                ['type' => 'amazon_s3'],
                [
                    'description' => json_encode($amazon_s3_data),
                    'updated_at' => now()
                ]
            );

        Session::flash('success', get_phrase('Amazon S3 settings updated successfully.'));
        return redirect()->route('admin.amazom_s3.settings');
    }
}
